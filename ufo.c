...
Version 0.91 SDL2 version
        0.92 resize added
        0.93 full screen in game and instructions
        0.94 full screen at startup for arcade cabinet
...
$ gcc -o ufo ufo.c -lSDL2 -lm -lSDL2_ttf -lSDL2_mixer

Windows (using MinGW):  TO DO
...
int full_screen = 0;
int first_startup = 1;      // full screen on first startup (for arcade cabinet)
...
void setup(void)
{
  ...
  if (SDL_Init(SDL_INIT_VIDEO) < 0)
    ...
    /* Open display: */
    if (first_startup == 0) {
      gWindow = SDL_CreateWindow( "UFO", SDL_WINDOWPOS_UNDEFINED, 
      ...
      screen = SDL_GetWindowSurface( gWindow );
       ...
       printf("current display w, h: %d, %d \n",dm.w, dm.h);
       
       factor = 5;
       //factor = trunc(dm.h / VIDEOPAC_RES_H);
       //if (factor > 9) factor = 9;
       
       screen_width  = dm.w; 
       screen_height = dm.h;

       //SDL_DestroyWindow( gWindow );     // close current window
       gWindow  = SDL_CreateWindow( "UFO",SDL_WINDOWPOS_CENTERED,
       ...
       //handle_screen_resize(4);  // reload images, fonts and reset objects etc
...
void load_images(void)
{
  ...
  for (i = 0; i <  NUM_IMAGES; i++)  
  {
    ...
    images[i] = SDL_ConvertSurfaceFormat(image, SDL_PIXELFORMAT_RGBA8888, 0);
    if (images[i] == NULL)  {
          fprintf(stderr,
    ...
    if (i != 10) {     // do not set transparency for white gun bit and bullets
        if (SDL_SetColorKey(images[i], (SDL_GetColorKey(images[i], NULL) | SDL_RLEACCEL),
...
int get_user_input()
{
    ...
    // Handle decrease / enlarge window (keypad + -), only relevant in non full screen mode  
    if (full_screen == 0) {
       if ( (keystate[SDL_SCANCODE_KP_MINUS] || keystate[SDL_SCANCODE_LEFTBRACKET] || keystate[SDL_SCANCODE_9]) && ship_dying != 1) { // decrease windows size
         if (factor > 1) {
           window_size_changed = 1;
           factor --;
           }
        }
       if ( (keystate[SDL_SCANCODE_KP_PLUS] || keystate[SDL_SCANCODE_RIGHTBRACKET] || keystate[SDL_SCANCODE_0]) && ship_dying != 1) { // increase windows size
         if (factor < 9) {
           window_size_changed = 1;
           factor ++;
           }
     ...
     /* Full screen is problematic in SDL2
        full screen          : use monitor screen resolution ans calculate nearest factor
        back from full screen: back to initial 1000x800 factor 5 */

     if (keystate[SDL_SCANCODE_8] && ship_dying != 1) { // toggle full_screen: 8 key

        if (full_screen == 1) {  // back to normal
            factor = 5;
            ...
            factor = trunc(dm.h / VIDEOPAC_RES_H);
            if (factor > 9) factor = 9;
            if (dm.h == 1080) factor = 7;   // correction for Full HD
            printf("factor is nu %d, \n", factor);
            screen_width  = dm.w; 
            screen_height = dm.h;

            handle_screen_resize(3);  // reload images, fonts and reset objects etc
     ...
     if (window_size_changed == 1) {  // normal resize 
            handle_screen_resize(2);
     }
  ...
  while (SDL_PollEvent(&event))
  {
   ...
   /* handle fire key */
   if ( (keystate[SDL_SCANCODE_LCTRL] || keystate[SDL_SCANCODE_RCTRL])  && ship_dying != 1 ) {
...
void handle_screen_resize(int mode)
{
  /* mode == 1: in instructions screen,   mode == 2: during game play
     mode == 3: full screen in game play, mode == 4: full screen in instructions 
  */
  int i;
  char title_string[100];
...
void draw_score_line() 
{
  ...
  text_position.x = (screen_width/2 - factor*78);
  //printf("x = %d\n", text_position.x);
  text_position.y = 140 * factor;
  ...
  text_position.x = (screen_width/2 - factor*78) + (3 * 12 * factor); // skip 3 chars
  text_position.y = 140 * factor;
  SDL_BlitSurface(text, NULL , screen, &text_position);
  ...
  if (ship_destroyed == 1) {
    ...
    text = TTF_RenderText_Solid(font_large, text_line, fgColor_green);
    text_position.x = (screen_width/2 - factor*78) + (4 * 12 * factor); // skip 4 chars
    text_position.y = 140 * factor;
    SDL_BlitSurface(text, NULL , screen, &text_position);
  }
  ...
  text_position.x = (screen_width/2 - factor*78) + (9 * 12 * factor); // skip 9 chars
  text_position.y = 140 * factor;
  SDL_BlitSurface(text, NULL , screen, &text_position);
...
void title_screen()
{
  ...
  do
  {
      ...
      if (event.type == SDL_KEYDOWN)
        {
          ...
          /* Handle screen resize, only when not in full screen */
           ...
           if (key == 56) { // toggle full_screen: 8 key
              ...
              if (full_screen == 1) {  // back to normal
                  ...
                  if (factor == 6) {  // full HD
                     asteroids[0].x = (screen_width / 2) - (75*factor);
                     asteroids[1].x = (screen_width / 2) - (22*factor);
                     ux = (screen_width / 2) + (35*factor);
                  } else {  
                     asteroids[0].x = (screen_width / 2) - (73*factor);
                     asteroids[1].x = (screen_width / 2) - (20*factor);
                     ux = (screen_width / 2) + (35*factor);
                  }                     
    ...
    if (frame - ufo_start_delay >= 30*3) {   // > 3 seconds delay for 1st ufo to appear
      ...
      // wobbly text!
      if (full_screen == 1) {
           x =  (screen_width / 2) - (12*4*factor) + ( (rand() % 4) - 2);
       } else {
...
void display_instructions(int scroll_x, int scroll_y)
{
  ...
  if (full_screen == 1)
     if (factor == 6)   // full HD
       text_position.x = (screen_width / 2) - (80*factor);
     else 
       text_position.x = (screen_width / 2) - (80*factor);
  else
